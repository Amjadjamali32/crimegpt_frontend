...
const Notifications = () => {
    ...
    const { userNotifications, loading, error } = useSelector((state) => state.notification);
    ...
    const loadMoreCards = () => {
        if (cardsToShow < userNotifications.length) {
            ...
            toast.info("No more notifications to load!", { position: "top-center", autoClose: 3000 });
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            toast.success("Notification deleted successfully!", { position: "top-center", autoClose: 3000 });
        } catch (err) {
            console.log(err);
            toast.error("Failed to delete notification", { position: "top-center", autoClose: 3000 });
        } finally {
            setIsDeleting(false);
            closeModal();
        }
    ...
    const confirmDeleteAll = async () => {
        ...
        try {
            ...
            toast.success("All notifications cleared!", { position: "top-center", autoClose: 3000 });
        } catch (err) {
            console.log(err);
            toast.error("Failed to delete all notifications", { position: "top-center", autoClose: 3000 });
        } finally {
    ...
    if (error) {
        return (
            <div>
                <button onClick={() => dispatch(fetchUserNotifications())}>Retry</button>
            </div>
        );
    }
            ...
            <div className="font-inter mb-4 text-sm sm:text-md grid grid-cols-1 gap-4 sm:ms-[40%] md:ms-[30%] lg:ms-[22%] xl:text-lg xl:ms-[22%]">
                ...
                {Array.isArray(userNotifications) && userNotifications.length === 0 ? (
                    <div className="text-center text-black">
                        <p className="text-center font-inter text-red-500">No notifications available!</p>
                    </div>
                ) : (
                    Array.isArray(userNotifications) && userNotifications.slice(0, cardsToShow).map((card) => (
                        <div
                            key={card._id}
                            className={`flex items-center rounded-sm shadow-xl p-4 cursor-pointer border ${
                                card.isRead || openedCards.has(card._id) ? "bg-gray-100" : "bg-gray-200"
                            }`}
                        ...
                        >
                            ...
                            <div className="ml-4 flex-grow">
                                <p className="text-gray-700 mt-0">{card.title.length > 30 ? `${card.title.slice(0, 30)}...` : card.title}</p>
                                <p className="text-sm text-gray-500">Date: {formatToPakistaniTime(card.createdAt)}</p>
                            </div>
                            <div className="ml-4">
                                <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(card._id); }}>
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className="fill-cyan-300">
                                        <path
                                            fillRule="evenodd"
                                            clipRule="evenodd"
                ...
                {userNotifications.length > cardsToShow && (
                    <div className="flex justify-center items-center">
                        <button className="w-full hover:opacity-80 bg-custom-teal text-white py-2 px-4 rounded-full shadow-md mt-4 md:w-2/6 sm:w-2/6 lg:w-1/6 xl:w-1/6" onClick={loadMoreCards}>
                            See More
                        </button>
                    </div>
                )}
